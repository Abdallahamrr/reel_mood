...
import 'package:hydrated_bloc/hydrated_bloc.dart'; // New
...
class MovieCubit extends Cubit<MovieState> {
  ...
  Future<Map<String, String>> fetchMovieDetails(int movieId) async {
    try {
      ...
      // Rounding the rating to 1 decimal place
      double rawRating = (response.data['vote_average'] as num).toDouble();

      return {
        'trailer_key': trailer['key'] ?? '',
        'imdb_id': response.data['imdb_id'] ?? '',
        'vote_average': rawRating.toStringAsFixed(1),
      };
    } catch (e) {
...
// --- WATCHLIST LOGIC (WITH PERSISTENCE) ---
class WatchlistCubit extends HydratedCubit<List<Map<String, dynamic>>> {
  ...
  List<Map<String, dynamic>>? fromJson(Map<String, dynamic> json) {
    return List<Map<String, dynamic>>.from(json['watchlist']);
  }
}
          ...
          return ElevatedButton(
            onPressed: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => MainNavigationWrapper(initialMood: chosenMood),
                ),
              );
            },
            ...
            return CardSwiper(
              ...
              onSwipe: (previousIndex, currentIndex, direction) {
                if (direction == CardSwiperDirection.right) {
                  ...
                  context.read<WatchlistCubit>().addToWatchlist(movie, selectedMood);
                }
                return true;
              },
                    ...
                    showModalBottomSheet(
                      ...
                      builder: (context) => Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(movie['title'], textAlign: TextAlign.center, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                            ...
                            ListTile(
                              ...
                              onTap: () => _launchURL('https://www.imdb.com/title/${details['imdb_id']}'),
                            ),
                            ListTile(
                              leading: const Icon(Icons.link, color: Colors.blue),
                              title: const Text('View on Rotten Tomatoes'),
                              onTap: () => _launchURL('https://www.rottentomatoes.com/search?search=${movie['title']}'),
                            ),
                          ],
                        ),
                      ),
    ...
    return Scaffold(
      ...
      body: BlocBuilder<WatchlistCubit, List<Map<String, dynamic>>>(
        builder: (context, watchlist) {
          if (watchlist.isEmpty) return const Center(child: Text("No movies saved yet!"));
          return ListView.builder(
            itemCount: watchlist.length,
            itemBuilder: (context, index) {
              final movie = watchlist[index];
              // Rounding the rating for consistent UI in the list
              String roundedRating = (movie['vote_average'] as num).toStringAsFixed(1);
              
              return ListTile(
                leading: Image.network('https://image.tmdb.org/t/p/w92${movie['poster_path']}'),
                title: Text(movie['title']),
                subtitle: Text('Rating: $roundedRating • ${movie['saved_mood']}'),
                trailing: IconButton(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            Text("Total Movies Liked: ${watchlist.length}", style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
            Text("Your Go-to Mood: $topMood", style: const TextStyle(fontSize: 18, color: Colors.greenAccent)),
          ],
        ),
      ),
    );
...
void _openTrailer(BuildContext context, String videoKey) async {
  if (videoKey.isEmpty) {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("No trailer found")));
    return;
  }
